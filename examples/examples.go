...
import (
	"fmt"
	gmssl3 "github.com/WandQ/GmSSL-Go"
)

func main() {
	fmt.Printf("GmSSL-Go Version: %s\n", gmssl3.GmSSLGoVersion)
	fmt.Printf("GmSSL Library Version: %s\n", gmssl3.GetGmSSLLibraryVersion())

	key, _ := gmssl3.RandBytes(16)
	fmt.Printf("RandBytes(32) : %x\n", key)

	fmt.Printf("Sm3DigestSize : %d\n", gmssl3.Sm3DigestSize)
	...
	sm4_cbc_dec, _ := gmssl3.NewSm4Cbc(key, iv, false)
	...
	sm4_ctr, _ := gmssl3.NewSm4Ctr(key, iv)
	...
	fmt.Printf("Sm4GcmMinIvSize = %d\n", gmssl3.Sm4GcmMinIvSize)
	fmt.Printf("Sm4GcmMinIvSize = %d\n", gmssl3.Sm4GcmMinIvSize)
	fmt.Printf("Sm4GcmDefaultIvSize = %d\n", gmssl3.Sm4GcmDefaultIvSize)
	fmt.Printf("Sm4GcmDefaultTagSize = %d\n", gmssl3.Sm4GcmDefaultTagSize)
	fmt.Printf("Sm4GcmMaxTagSize = %d\n", gmssl3.Sm4GcmMaxTagSize)
	aad, _ := gmssl3.RandBytes(20)
	taglen := gmssl3.Sm4GcmDefaultTagSize
	iv, _ = gmssl3.RandBytes(gmssl3.Sm4GcmDefaultIvSize)

	sm4_gcm_enc, _ := gmssl3.NewSm4Gcm(key, iv, aad, taglen, true)
	...
	fmt.Printf("ciphertext = %x\n", gcm_ciphertext)
	sm4_gcm_dec, _ := gmssl3.NewSm4Gcm(key, iv, aad, taglen, false)
	...
	iv, _ = gmssl3.RandBytes(gmssl3.ZucIvSize)

	zuc, _ := gmssl3.NewZuc(key, iv)
	...
	fmt.Printf("Sm2MaxSignatureSize = %d\n", gmssl3.Sm2MaxSignatureSize)
	fmt.Printf("Sm2MinPlaintextSize = %d\n", gmssl3.Sm2MinPlaintextSize)
	fmt.Printf("Sm2MaxPlaintextSize = %d\n", gmssl3.Sm2MaxPlaintextSize)
	fmt.Printf("Sm2MinCiphertextSize = %d\n", gmssl3.Sm2MinCiphertextSize)
	fmt.Printf("Sm2MaxCiphertextSize = %d\n", gmssl3.Sm2MaxCiphertextSize)

	sm2, _ := gmssl3.GenerateSm2Key()
	sm2.ExportEncryptedPrivateKeyInfoPem("Password", "sm2.pem")
	sm2.ExportPublicKeyInfoPem("sm2pub.pem")
	sm2pri, _ := gmssl3.ImportSm2EncryptedPrivateKeyInfoPem("Password", "sm2.pem")
	sm2pub, _ := gmssl3.ImportSm2PublicKeyInfoPem("sm2pub.pem")
	z, _ := sm2pub.ComputeZ(gmssl3.Sm2DefaultId)
	fmt.Printf("Z = %x\n", z)
	Z, _ := sm2pri.ComputeZ(gmssl3.Sm2DefaultId)
	fmt.Printf("Z = %x\n", Z)
	...
	sign, _ := gmssl3.NewSm2Signature(sm2pri, gmssl3.Sm2DefaultId, true)
	sign.Update([]byte("abc"))
	signature1, _ := sign.Sign()
	fmt.Printf("Sm2Signature Signature = %x\n", signature1)

	sign, _ = gmssl3.NewSm2Signature(sm2pub, gmssl3.Sm2DefaultId, false)
	sign.Update([]byte("abc"))
	...
	fmt.Printf("Sm9MaxPlaintextSize = %d\n", gmssl3.Sm9MaxPlaintextSize)
	fmt.Printf("Sm9MaxCiphertextSize = %d\n", gmssl3.Sm9MaxCiphertextSize)
	fmt.Printf("Sm9SignatureSize = %d\n", gmssl3.Sm9SignatureSize)

	sm9_enc_master, _ := gmssl3.GenerateSm9EncMasterKey()
	sm9_enc_master.ExportEncryptedMasterKeyInfoPem("sm9enc.pem", "password")
	sm9_enc_master.ExportMasterPublicKeyPem("sm9encpub.pem")

	sm9_enc_master, _ = gmssl3.ImportEncryptedSm9EncMasterKeyInfoPem("sm9enc.pem", "password")
	sm9_enc_master_pub, _ := gmssl3.ImportSm9EncMasterPublicKeyPem("sm9encpub.pem")
	...
	sm9_enc_key, _ = gmssl3.ImportEncryptedSm9EncPrivateKeyInfoPem("sm9encpri.pem", "password", "Alice")

	sm9_plaintext, _ := sm9_enc_key.Decrypt(sm9_ciphertext)
	fmt.Printf("SM9 Plaintext : %s\n", sm9_plaintext)

	sm9_sign_master, _ := gmssl3.GenerateSm9SignMasterKey()
	...
	sm9_sign_master, _ = gmssl3.ImportEncryptedSm9SignMasterKeyInfoPem("sm9sign.pem", "password")
	sm9_sign_master_pub, _ := gmssl3.ImportSm9SignMasterPublicKeyPem("sm9signpub.pem")

	sm9_sign_key, _ := sm9_sign_master.ExtractKey("Alice")
	fmt.Printf("Id : %s\n", sm9_sign_key.GetId())
	sm9_sign_key.ExportEncryptedPrivateKeyInfoPem("sm9signpri.pem", "password")
	sm9_sign_key, _ = gmssl3.ImportEncryptedSm9SignPrivateKeyInfoPem("sm9signpri.pem", "password", "Alice")

	sm9_sign, _ := gmssl3.NewSm9Signature(true)
	sm9_sign.Update([]byte("abc"))
	sm9_signature, _ := sm9_sign.Sign(sm9_sign_key)
	fmt.Printf("SM9 Signature : %x\n", sm9_signature)

	sm9_verify, _ := gmssl3.NewSm9Signature(false)
	...
	/*
	cert, _ := gmssl3.ImportSm2CertificatePem("ROOTCA.pem")
	...
	cert_verify_ret := cert.VerifyByCaCertificate(cert, gmssl3.Sm2DefaultId)
